...
export async function getRandomQuotes(params, next) {
  try {
    ...
    if (tags) {
      $match['tags._id'] = getTagsFilter(tags)
    }
    ...
    const results = await Quotes.aggregate(
      compact([
        ...
        { $project: { _id : 1, content : 1, tags: 1, authorId: 1 } },
    ...
    const test = await Authors.find(
        {
          '_id': {
            '$in': results.map((result) => result.authorId)
          }
        },
        {
          _id: 1,
    ...
    return await Promise.all(results.map(async (result) => ({
        ...
        tags : result.tags,
        author: test.find(x => x._id === result.authorId)
      })))
  } catch (error) {
    return next(error)
  }
